...
* Postorder traveral (Left Right Node)->print according to this.
*/
...
* By using the Postorder traversal which firstly traverse to the left.
* then  go to the right side of the binary tree and finally print the .
* node value of the binary tree. 
* @param root store's the first node address
* @return if the root ==null , "void"
*/
void Postorder_biaryTree(node *root){
	if(root == nullptr) return;
	Postorder_biaryTree(root->left);
	Postorder_biaryTree(root->right);
	std::cout<<root->data<<" ";
}
...
* user can print the binary tree using Postorder (LRN)
*/
int main(){
