...
* inorder traveral (Left Node Right)->print according to this.
*/
...
* By using the inorder traversal which firstly traverse to the left.
* then print the node and finally go to the right side of the binary tree.
* @param root store's the first node address
* @return if the root ==null , "void"
*/
void Inorder_biaryTree(node *root){
	if(root == nullptr) return;
	Inorder_biaryTree(root->left);
	std::cout<<root->data<<" ";
	Inorder_biaryTree(root->right);
}
...
* user can print the binary tree using inorder (LNR)
*/
int main(){
